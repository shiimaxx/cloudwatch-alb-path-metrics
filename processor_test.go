...
func TestShouldPrintLogLine_Match(t *testing.T) {
	...
	assert.True(t, shouldPrintLogLine(line, rules))
}

func TestShouldPrintLogLine_NoMatch(t *testing.T) {
	rules, err := newPathRules(`[{"host":"api.example.com","path":"^/users/[0-9]+$","route":"/users/:id"}]`)
	require.NoError(t, err)
	...
	assert.False(t, shouldPrintLogLine(line, rules))
}

func TestShouldPrintLogLine_ParseError(t *testing.T) {
	rules, err := newPathRules(`[{"host":"api.example.com","path":"^/users/[0-9]+$","route":"/users/:id"}]`)
	require.NoError(t, err)

	assert.False(t, shouldPrintLogLine("invalid", rules))
}
